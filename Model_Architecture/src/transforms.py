...
class Chroma(object):
    ...
    def __init__(self, n_fft=2048, hop=160, fft_type="stft",
                 n_chroma=12, sr=16000, win=400, name='chroma'):
        ...
        #self.win = win
        self.n_fft = win
        # cqt would require hop_length to be integer multiple of 2^6 ==> use stft first
    ...
    def __call__(self, pkg, cached_file=None):
        ...
        if cached_file is not None:
            # load pre-computed data
            chroma = torch.load(cached_file)
            beg_i = pkg['chunk_beg_i'] // self.hop
            end_i = pkg['chunk_end_i'] // self.hop
            chroma = chroma[:, beg_i:end_i]
            pkg[self.name] = chroma
        else:
            # print(y.dtype)
            chroma = librosa.feature.chroma_stft(y, sr=self.sr,
                                        n_fft=self.n_fft,
                                        hop_length=self.hop,
                                        #win_length=self.win,
            ...
            pkg[self.name] = torch.tensor(chroma.astype(np.float32))
        # Overwrite resolution to hop length
        pkg['dec_resolution'] = self.hop
        return pkg

    def __repr__(self):
        attrs = '(fft_type={}, sr={})'.format(self.fft_type, self.sr)
        return self.__class__.__name__ + attrs

class Tempogram(object):

    def __init__(self, hop=160,
                 sr=16000, win=400, name='tempogram'):
    ...
    def __call__(self, pkg, cached_file=None):
        ...
        if cached_file is not None:
            # load pre-computed data
            tempogram = torch.load(cached_file)
            beg_i = pkg['chunk_beg_i'] // self.hop
            end_i = pkg['chunk_end_i'] // self.hop
            tempogram = tempogram[:, beg_i:end_i]
            pkg[self.name] = tempogram
        else:
            # print(y.dtype)
            ...
            pkg[self.name] = torch.tensor(tempogram.astype(np.float32))
        # Overwrite resolution to hop length
        pkg['dec_resolution'] = self.hop
        return pkg

    def __repr__(self):
        attrs = '(hop length={}, sr={})'.format(self.hop, self.sr)
        return self.__class__.__name__ + attrs
