...
# echo "norm_label_lds" >> $correlation_results_file
# for seed in ${seeds[@]}; do
#     correlation_results=($seed)
#     loss_results=($seed)
#     for fold in "${folds[@]}"; do
#         exp_name="seed_${seed}_fold_${fold}_norm_label_lds"
#         echo "trainig ${exp_name} with ${model}..."
...
# with augmentation, LDS

# echo "norm_label_aug_lds" >> $correlation_results_file
# echo "norm_label_aug_lds" >> $loss_results_file
# for seed in ${seeds[@]}; do
#     correlation_results=($seed)
#     loss_results=($seed)
#     for fold in "${folds[@]}"; do
#         exp_name="seed_${seed}_fold_${fold}_norm_label_aug_lds"
#         echo "trainig ${exp_name} with ${model}..."
#         python train.py --model $model --name $exp_name --kfold_splits 10 --fold_index $fold --seed $seed --use_pitch_shift --use_lds True
#         echo "testing ${exp_name} with ${model}..."
#         python test.py --model $model --fold_index $fold --load "weights/${model}/${exp_name}/${model}_best.pth" --outdir "results/${model}/${exp_name}"
#         # read ouptut file store fold results to array
#         exec < "results/${model}/${exp_name}/details.txt"
